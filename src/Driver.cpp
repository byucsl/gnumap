...
// was designed to only work with 8 threads. Working on a workaround now.
...
// if we're doing the needleman-wunsch counting for deubgging
#ifdef DEBUG_NW
    num_nw = new unsigned int[ gNUM_THREADS ];
    ...
    if( gILLUMINA )
    {
        params << "\tUsing old illumina scores." << endl;
    }
    else
    {
        params << "\tUsing new phred quality scores." << endl;
    }
    ...
    of.open( sam_file, std::ofstream::out | std::ofstream::binary );
...
void single_write_cond_wait(int thread_no) {
    ...
    cerr << "thread id:\t" << thread_no << "\twriting in function!\t" << gTopReadOutput.size() << endl;
    ...
    for(vit = gTopReadOutput.begin(); vit != gTopReadOutput.end(); vit++) {
        /*MUTEX_LOCK( &output_lock );
        cerr << "thread id:" << thread_no << "\tloop!" << endl;
        cerr << "\tfh open?\t" << of.is_open() << endl;
        cerr << "\tgood?\t" << of.good() << endl;
        cerr << "\teof?\t" << of.eof() << endl;
        cerr << "\trdstate?\t" << of.rdstate() << endl;
        cerr << "\t" << (*vit).READ_NAME << "\t" << (*vit).strand << "\t" << (*vit).CIGAR << endl;
        MUTEX_UNLOCK( &output_lock );*/
...
void* parallel_thread_run(void* t_opts) {
    ...
    if( thread_id == 0 )
    {
        ...
        {
            of << "@SQ\tSN:" << ( *header_it ).first << "\tLN:" << ( ( * ( header_it + 1 ) ).second - ( *header_it ).second ) << "\n";
        }
...
int set_arg_ext(const char* param, const char* assign, int &count) {
    ...
    switch(which) {    //switch on the first character
        ...
        case PAR_A_TO_G:
            ...
            break;
        case PAR_ILL:
            //cerr << "I'M SETTING THE ILLUMINA FLAG :(*************************" << endl;
            gILLUMINA = true;
            break;
        case PAR_LARGEMEM:
            gMPI_LARGEMEM = true;
