...
GenomeSUFFIX::GenomeSUFFIX() : Genome(), hash_data(0) {
#ifdef MAP_USE_STRING
	fprintf(stderr, "# Using SUFFIX genome type: string\n");
#else
	fprintf(stderr, "# Using SUFFIX genome type: unsigned int\n");
#endif
}

GenomeSUFFIX::GenomeSUFFIX(const char* fn) : Genome(fn), hash_data(0) {
#ifdef MAP_USE_STRING
	fprintf(stderr, "Using SUFFIX genome type: string\n");
#else
	fprintf(stderr, "Using SUFFIX genome type: unsigned int\n");
...
GenomeSUFFIX::~GenomeSUFFIX() {
	if(hash_data)
		delete[] hash_data;
}

void GenomeSUFFIX::print_to_file(map<MAP_TYPE, HashLocation> & gh, char* ofn) { 
...
void GenomeSUFFIX::readHash(FILE* readF, bool ldebug) {
...
void GenomeSUFFIX::hash_and_store() {
...
void GenomeSUFFIX::convertToVector(map<MAP_TYPE, vector<unsigned long> > *gh, 
							 gvector<GEN_TYPE> *pg) {
...
/********************************/
void GenomeSUFFIX::saveHash(FILE* saveF, bool ldebug) {
...
void GenomeSUFFIX::fix_hash(map<MAP_TYPE, vector<unsigned long> > *gh, 
							 gvector<GEN_TYPE> *pg) {
...
HashLocation* GenomeSUFFIX::GetMatches(string &str) {
#ifdef MAP_USE_STRING
...
HashLocation* GenomeSUFFIX::GetMatches(unsigned int hash) {
#ifdef MAP_USE_STRING
